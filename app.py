...
def render_login():
    ...
    if request.method == 'POST':
      ...
      if(enc_pass == account["password"]):
          ...
          company_name_list             =dbo.selected_instrument_dropdown("AIR_VELOCITY",session_var['user'],account["role"]).COMPANY_NAME.unique().tolist()
          equipment_list                =dbo.selected_instrument_dropdown("AIR_VELOCITY",session_var['user'],account["role"]).SR_NO_ID.unique().tolist()
          usernameList =[]
          if account["role"] == "admin":
              customer_details=dbo.get_company_details()
              company_name_list = customer_details.COMPANY_NAME.unique().tolist()
              ret_page = "ADMIN/add_user.html"
          elif account["role"] == "analyst":
              ret_page = "HVAC_UI/Air_velocity.html"
          elif account["role"] == "operation":
              ret_page = "ELOGBOOK/approve_instrument_request.html"
          elif account["role"] == "companyPortal":
              ret_page = "ADMIN/download_reportcompany.html"
              usernameList  = dbo.get_username()
              company_name_list =[account["COMPANYNAME"]]
          else:
              ret_page = "HVAC_UI/Thermal.html"
              company_name_list =["abc"]
          return make_response(render_template(ret_page,company_list=company_name_list,
                                grade_list = grade_list,
                                equipment_list = equipment_list,user_type='admin',usernameList=usernameList,
                                msg = True, err = False, warn = False, role = session_var["role"]),200)
      else:
...
def add_user_page():
    if 'user' in session:
        ...
        return make_response(render_template('ADMIN/add_user.html',company_list=company_name_list,role = role),200) 
    return make_response(render_template('LOGIN_PAGE/login.html'),200)
...
def render_Air_velocity():
    if 'user' in session:
        ...
        role = session_var["role"]
        #customer_details              = dbo.get_company_details()        
        company_name_list             = dbo.selected_instrument_dropdown("AIR_VELOCITY",session_var['user'],role).COMPANY_NAME.unique().tolist()
        equipment_list                = dbo.selected_instrument_dropdown("AIR_VELOCITY",session_var['user'],role).SR_NO_ID.unique().tolist()
...
def render_paotest():
    if 'user' in session:
        ...
        company_name_list             = dbo.selected_instrument_dropdown("PAO_TEST",session_var['user'],role).COMPANY_NAME.unique().tolist()
        equipment_list = dbo.selected_instrument_dropdown("PAO_TEST",session_var['user'],role).SR_NO_ID.unique().tolist()
...
def close_instrument_request():
    if 'user' in session:
        session_var = session['user']
        role        = session_var["role"]
        equipment_details   = dbo.get_approved_instrument(session_var['user'])
        equipment_list      = equipment_details.to_dict('records')
        return make_response(render_template('ELOGBOOK/close_instrument_request.html',equipment_list  = equipment_list, role = role),200) 
    return make_response(render_template('LOGIN_PAGE/login.html'),200)   
...
def push_instrument_request():
    if 'user' in session:
        ...
        for equipment in observation['selected_eq']:
            #print(equipment)
            dbo.request_for_equipment(str(equipment),observation['company_name'],
                                 observation['REMARK'],session_var['user'])
        d = {"error":"none"}
        return json.dumps(d) 
    return make_response(render_template('LOGIN_PAGE/login.html'),200)        
...
def download_report ():
    if 'user' in session:
        ...
        return make_response(render_template('ADMIN/download_report.html',company_list=company_name_list,usernameList=usernameList,role = role),200) 
    return make_response(render_template('LOGIN_PAGE/login.html'),200) 
